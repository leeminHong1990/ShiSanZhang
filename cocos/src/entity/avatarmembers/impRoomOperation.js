...
var impRoomOperation = impGameRules.extend({
    ...
    uploadPokerResult: function (poker_list) {
        cc.log("uploadPokerResult");
        this.baseCall("uploadPokerResult", poker_list);
    },

    uploadPokerResultFailed: function (err) {
        cc.log("uploadPokerResultFailed", err);
    },

    giveControl:function (idx) {
        if(!this.curGameRoom){
            return;
        }
        this.curGameRoom.updatePlayerCharacter(idx, const_val.CHARACTER_ADMIN)
		this.curGameRoom.updatePlayerState(idx, 0)
		// ui 变化
        if(h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show){
            h1global.curUIMgr.gameroomprepare_ui.show_start_btn();
		...
		if(this.curGameRoom){
			this.curGameRoom.playerInfoList[serverSitNum] = null;
			 if(h1global.curUIMgr.gameroom_ui && h1global.curUIMgr.gameroom_ui.is_show){
			 	h1global.curUIMgr.gameroom_ui.update_player_info_panel(serverSitNum, this.curGameRoom.playerInfoList[serverSitNum]);
			 }
		}
	},
	...
	othersEnterRoom : function(playerInfo){
		...
		if(h1global.curUIMgr.gameroom_ui && h1global.curUIMgr.gameroom_ui.is_show){
            h1global.curUIMgr.gameroom_ui.update_player_info_panel(playerInfo["idx"], playerInfo);
            h1global.curUIMgr.gameroom_ui.update_player_game_state(playerInfo["idx"]);
		}
		if(h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show){
            h1global.curUIMgr.gameroomprepare_ui.check_invition()
		}
	...
	applyDismissRoom : function(){
		if(this.curGameRoom){
			if(this.curGameRoom.playerInfoList[this.serverSitNum]["character"] == const_val.CHARACTER_VISITOR){
				return
			}
	...
	reqDismissRoom : function(serverSitNum){
		cc.log("reqDismissRoom", serverSitNum);
		if(this.curGameRoom){
	...
	voteDismissResult : function(serverSitNum, vote){
		...
		if(this.curGameRoom){
			...
			for(var i = 0; i < this.curGameRoom.playerInfoList.length; i++){
				if(this.curGameRoom.playerInfoList[i] && this.curGameRoom.playerInfoList[i]["character"] !== const_val.CHARACTER_VISITOR){
                    vote_num += 1
				}
            ...
            if (vote_disagree_num >= 2 ||  (vote_disagree_num >= 1 && vote_num < this.curGameRoom.playerInfoList.length)) { // 继续游戏
                ...
                return;
            } else if (vote_agree_num === vote_num || (vote_agree_num === vote_num + 1 && vote_num === this.curGameRoom.playerInfoList.length)) { // 退出房间
                this.quitRoom();
            }
			if(h1global.curUIMgr.applyclose_ui && h1global.curUIMgr.applyclose_ui.is_show){
