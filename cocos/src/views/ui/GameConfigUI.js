...
var GameConfigUI = UIBase.extend({
	...
	change_color:function () {
        var gameBgType = cc.sys.localStorage.getItem("GAME_ROOM_BG");
        if(h1global.curUIMgr.updateBackground){
            h1global.curUIMgr.updateBackground(cc.sys.localStorage.getItem("GAME_ROOM_UI"),gameBgType);
		}
    },

    change_mode:function () {
		var gameType = cc.sys.localStorage.getItem("GAME_ROOM_UI");
    },
	...
	update_mode_color:function () {
			...
			if(eventType == ccui.Widget.TOUCH_ENDED){
				for(var i = 0 ; i < self.mode_btn_list.length ; i++){
					if(sender != self.mode_btn_list[i]){
						...
						cc.sys.localStorage.setItem("GAME_ROOM_UI",i);
						self.change_mode();
					}
				}
			}
        ...
        let btn_mode = cc.sys.localStorage.getItem("GAME_ROOM_UI");
            ...
            if(eventType == ccui.Widget.TOUCH_ENDED){
                for(var i = 0 ; i < self.color_btn_list.length ; i++){
                    if(sender != self.color_btn_list[i]){
                        ...
                        cc.sys.localStorage.setItem("GAME_ROOM_BG",i);
                        self.change_color();
                    }
                }
            }
        ...
        let bg_mode = cc.sys.localStorage.getItem("GAME_ROOM_BG");
