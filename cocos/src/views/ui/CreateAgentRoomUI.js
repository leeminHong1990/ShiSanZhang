...
var CreateAgentRoomUI = UIBase.extend({
    ...
    initCreateRoom:function(){
        ...
        function create_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
				...
				var game_mode = parameters["game_mode"];
				var round_num = parameters["round_num"];
				var max_lose = parameters["max_lose"];
				var treasure_num = parameters["treasure_num"];
				var discard_seconds = parameters["discard_seconds"];
				var hand_prepare = parameters["hand_prepare"];
				cc.log("CreateAgentRoom args: ", parameters);
				h1global.player().createRoom(game_mode, pay_mode, round_num, max_lose, treasure_num,
					discard_seconds, hand_prepare, 1);
            }
        }
    ...
    updatePlayingItem: function (item, data) {
    	...
    	var cur_round = data['current_round'];
    	var max_round = data['game_round'];
        ...
        if (cur_round >= 1) {
			...
			var share_title = '十三张 房间号【' + room_id.toString() + '】' + "【代开房】";
