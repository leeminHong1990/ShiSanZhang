    ...
    initUI:function(){
        ...
        this.freecard_panel.getChildByName("friend_btn").addTouchEventListener(function(sender, eventType) {
            ...
            if(eventType == ccui.Widget.TOUCH_ENDED) {
                ...
                var info_dict = eval('(' + cc.sys.localStorage.getItem("INFO_JSON") + ')');
                if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative)){
                    jsb.reflection.callStaticMethod(switches.package_name + "/AppActivity", "shareZQM", "(ZLjava/lang/String;)V", true, "http://gxmj.zjfeixia.com/invite/" + info_dict["user_id"]);
                } else if((cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
                    jsb.reflection.callStaticMethod("WechatOcBridge", "shareZQM:withURL:", true, "http://gxmj.zjfeixia.com/invite/" + info_dict["user_id"]);
                } else {
                    cutil.share_func(switches.gameName + "赚钱码", "专属赚钱码下载并使用游戏，自动绑定上下级关系。每成功邀请一名玩家，奖励5元红包。");
                    h1global.curUIMgr.share_ui.show();
                }
            }
        });
        ...
        this.freecard_panel.getChildByName("friends_btn").addTouchEventListener(function(sender, eventType) {
            ...
            if(eventType == ccui.Widget.TOUCH_ENDED) {
                ...
                var info_dict = eval('(' + cc.sys.localStorage.getItem("INFO_JSON") + ')');
                if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative)){
                    jsb.reflection.callStaticMethod(switches.package_name + "/AppActivity", "shareZQM", "(ZLjava/lang/String;)V", false, "http://gxmj.zjfeixia.com/invite/" + info_dict["user_id"]);
                } else if((cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
                    jsb.reflection.callStaticMethod("WechatOcBridge", "shareZQM:withURL:", false, "http://gxmj.zjfeixia.com/invite/" + info_dict["user_id"]);
                } else {
                    cutil.share_func(switches.gameName + "赚钱码", "专属赚钱码下载并使用游戏，自动绑定上下级关系。每成功邀请一名玩家，奖励5元红包。");
                    h1global.curUIMgr.share_ui.show();
                }
