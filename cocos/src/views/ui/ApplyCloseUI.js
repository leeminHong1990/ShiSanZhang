...
var ApplyCloseUI = BasicDialogUI.extend({
	...
	initUI:function(){
		...
		//先隐藏4个框
        this.applyclose_panel.getChildByName("player_info_panel0").setVisible(false);
        this.applyclose_panel.getChildByName("player_info_panel1").setVisible(false);
        this.applyclose_panel.getChildByName("player_info_panel2").setVisible(false);
	...
	update_vote_state : function(){
		...
		for(var i = 0; i < this.player_info_panels_list.length; i++){
	...
	show_by_sitnum:function(serverSitNum){
		...
		this.show(function(){
			...
			if(serverSitNum == player.serverSitNum || player.curGameRoom.applyCloseStateList[player.serverSitNum] != 0){
