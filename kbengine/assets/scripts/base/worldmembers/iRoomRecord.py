...
class iRoomRecord(object):
	...
	def begin_record_room(self, mailbox, room_id):
		rid = self.recordIndex + 1
		self.recordIndex = rid
		self.roomRecordIdDict[room_id] = rid

		players_list = mailbox.players_list
		init_pokers = [None] * len(players_list)
		player_id_list = []
		for i,p in enumerate(players_list):
			if p is not None:
				init_pokers[p.idx] = list(p.pokers)
				player_id_list.append(p.mb.userId)

		record = {
			'recordId': rid,
			'init_info': mailbox.get_init_client_dict(),
			'player_id_list': player_id_list,
			'init_pokers': init_pokers,
